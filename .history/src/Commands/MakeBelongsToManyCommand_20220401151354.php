...
class MakeBelongsToManyCommand extends Command
{
    ...
    protected $signature   = 'zake:dasher-belongs-to-many {resource?} {relationship?} {recordTitleAttribute?} {--F|force}';
    ...
    public function handle() : int
    {
        ...
        $path = \app_path(
            (string) Str::of($managerClass)
                ->prepend("Dashboards\\Resources\\{$resource}\\RelationManagers\\")
                ->replace('\\', '/')
                ->append('.php'),
        );
        ...
        $this->copyStubToApp('BelongsToManyRelationManager', $path, [
            'namespace'            => "App\\Dashboards\\Resources\\{$resource}\\RelationManagers",
