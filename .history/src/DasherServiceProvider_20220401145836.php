...
class DasherServiceProvider extends PackageServiceProvider
{
    ...
    public function packageRegistered(): void
    {
        ...
        $this->app->scoped('dasher', function (): DashboardManager {
            return app(DashboardManager::class);
        });
    ...
    protected function discoverPages(): void
    {
        ...
        Filament::registerPages(collect($filesystem->allFiles(config('dasher.pages.path')))
    ...
    protected function discoverResources(): void
    {
        ...
        Filament::registerResources(config('dasher.resources.register', []));

        if (! $filesystem->exists(config('dasher.resources.path'))) {
            return;
        }

        Filament::registerResources(collect($filesystem->allFiles(config('dasher.resources.path')))
            ->map(function (SplFileInfo $file): string {
    ...
    protected function discoverWidgets(): void
    {
        ...
        Filament::registerWidgets(config('dasher.widgets.register', []));

        if (! $filesystem->exists(config('dasher.widgets.path'))) {
            return;
        }

        Filament::registerWidgets(collect($filesystem->allFiles(config('dasher.widgets.path')))
            ->map(function (SplFileInfo $file): string {
