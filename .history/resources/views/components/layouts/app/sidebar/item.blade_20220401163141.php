...
<li class="dasher-sidebar-item">
  <a href="{{ $url }}" {!! $shouldOpenUrlInNewTab ? 'target="_blank"' : '' !!}
    x-on:click="window.matchMedia(`(max-width: 1024px)`).matches && $store.sidebar.close()"
    @if (config('dasher.layout.sidebar.is_collapsible_on_desktop')) x-data="{ tooltip: {} }"
            x-init="
                Alpine.effect(() => {
    ...
    @class([
        ...
        'dark:text-gray-300 dark:hover:bg-gray-700' =>
            !$active && config('dasher.dark_mode'),
        'bg-primary-500 text-white' => $active,
    ])>
    <x-dynamic-component :component="$icon" class="h-5 w-5 shrink-0" />

    <div class="flex flex-1"
      @if (config('dasher.layout.sidebar.is_collapsible_on_desktop')) x-data="{}"
                x-show="$store.sidebar.isOpen" @endif>
      <span>
        {{ $slot }}
      </span>
      ...
      @if (filled($badge))
        <span @class([
            ...
            'dark:text-primary-500' => !$active && config('dasher.dark_mode'),
        ])>
          {{ $badge }}
        </span>
      @endif
