...
<div x-data="{
  ...
  <div x-show="isOpen" x-on:click.away="isOpen = false" x-transition:enter="transition"
    ...
    <ul @class([
      ...
      <x-filament::dropdown.item :color="$accountItem?->getColor() ?? 'secondary'"
      ...
      @foreach ($items as $key => $item)
        @if ($key !== 'account' && $key !== 'logout')
          <x-filament::dropdown.item :color="$item->getColor() ?? 'secondary'"
      ...
      <x-filament::dropdown.item :color="$logoutItem?->getColor() ?? 'secondary'"
        :icon="$logoutItem?->getIcon() ?? 'heroicon-s-logout'"
        :action="$logoutItem?->getUrl() ?? route('filament.auth.logout')" method="post" tag="form">
