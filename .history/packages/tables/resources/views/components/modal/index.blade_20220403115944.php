...
<div x-data="{ isOpen: false }"
  ...
  role="dialog" aria-modal="true" class="{{ $displayClasses }} dasher-tables-modal">
  {{ $trigger }}
  ...
  <div x-show="isOpen" x-transition:enter="ease duration-300" x-transition:enter-start="opacity-0"
    ...
    <button x-on:click="isOpen = false" type="button" aria-hidden="true"
      class="fixed inset-0 w-full h-full bg-black/50 focus:outline-none dasher-tables-modal-close-overlay"></button>
    ...
    <div x-show="isOpen" x-trap="isOpen" x-on:keydown.window.escape="isOpen = false"
      ...
      <div @class([
          'w-full mx-auto p-2 space-y-2 bg-white rounded-xl cursor-default dasher-tables-modal-window',
        ...
        @if ($header)
          <div class="px-4 py-2 dasher-tables-modal-header">
            {{ $header }}
          </div>
        @endif
        ...
        <div class="space-y-2 dasher-tables-modal-content">
        ...
        @if ($footer)
          <div class="px-4 py-2 dasher-tables-modal-footer">
            {{ $footer }}
          </div>
        @endif
      </div>
