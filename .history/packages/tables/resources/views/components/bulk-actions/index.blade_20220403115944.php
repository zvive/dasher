...
<div x-data="{
  ...
  {{ $attributes->class(['relative dasher-tables-bulk-actions']) }}>
  ...
  <div x-show="isOpen" x-on:click.away="isOpen = false" x-transition:enter="transition"
    ...
    <ul @class([
      ...
      @foreach ($actions as $action)
        <x-tables::dropdown.item :x-on:click="'mountBulkAction(\'' . $action->getName() . '\')'"
          :icon="$action->getIcon()" :color="$action->getColor()">
          {{ $action->getLabel() }}
