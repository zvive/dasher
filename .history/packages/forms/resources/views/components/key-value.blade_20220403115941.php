<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()"
  ...
  <div x-data="keyValueFormComponent({
      state: $wire.{{ $applyStateBindingModifiers('entangle(\'' . $getStatePath() . '\')') }},
  })"
    {{ $attributes->merge($getExtraAttributes())->class(['dasher-forms-key-value-component']) }}
    ...
    <div @class([
        ...
        <tbody x-ref="tableBody" @class([
          ...
          <template x-for="(row, index) in rows" x-bind:key="index" x-ref="rowTemplate">
            <tr @class([
              ...
              <td>
                <input type="text" x-model="row.key" x-on:input="updateState"
                  {!! ($placeholder = $getKeyPlaceholder()) ? "placeholder=\"{$placeholder}\"" : '' !!} @if (!$canEditKeys() || $isDisabled()) disabled @endif
                  class="w-full px-4 py-3 font-mono text-sm bg-transparent border-0 focus:ring-0">
              </td>

              <td class="whitespace-nowrap">
                <input type="text" x-model="row.value" x-on:input="updateState"
                  {!! ($placeholder = $getValuePlaceholder()) ? "placeholder=\"{$placeholder}\"" : '' !!} @if (!$canEditValues() || $isDisabled()) disabled @endif
                  class="w-full px-4 py-3 font-mono text-sm bg-transparent border-0 focus:ring-0">
              </td>
              ...
              @if ($canDeleteRows() && $isEnabled())
                <td x-show="rows.length > 1" class="whitespace-nowrap">
                  <div class="flex items-center justify-center">
                    <button x-on:click="deleteRow(index)" type="button"
      ...
      @if ($canAddRows() && $isEnabled())
        <button x-on:click="addRow" type="button" @class([
