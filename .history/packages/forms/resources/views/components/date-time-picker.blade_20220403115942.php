@once
  @push('scripts')
    ...
    <script defer src="//unpkg.com/dayjs@1.10.4/locale/{{ $locale }}.js"
        onload="dayjs.updateLocale('{{ $locale }}')"></script>
  @endpush
@endonce
...
<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()"
  ...
  <div x-data="dateTimePickerFormComponent({
    ...
    x-on:keydown.escape.stop="closePicker()" x-on:blur="closePicker()"
    {{ $attributes->merge($getExtraAttributes())->class(['relative dasher-forms-date-time-picker-component']) }}
    {{ $getExtraAlpineAttributeBag() }}>
    <button
      @unless($isDisabled()) x-ref="button"
                ...
                dusk="dasher.forms.{{ $getStatePath() }}.open" @endunless
      type="button"
      ...
      <input readonly placeholder="{{ $getPlaceholder() }}" x-model="displayText"
        {!! ($id = $getId()) ? "id=\"{$id}\"" : null !!} @class([
    ...
    @unless($isDisabled())
      <div x-ref="picker" x-on:click.away="closePicker()" x-show.transition="open" aria-modal="true"
        role="dialog" x-cloak @class([
        ...
        <div class="space-y-3">
          @if ($hasDate())
            <div class="flex items-center justify-between space-x-1 rtl:space-x-reverse">
              <select x-model="focusedMonth" @class([
                ...
                dusk="dasher.forms.{{ $getStatePath() }}.focusedMonth">
              ...
              <input type="number" inputmode="numeric" x-model.debounce="focusedYear"
                ...
                dusk="dasher.forms.{{ $getStatePath() }}.focusedYear" />
            </div>

            <div class="grid grid-cols-7 gap-1">
              <template x-for="(day, index) in dayLabels" :key="index">
                <div x-text="day" @class([
            ...
            <div role="grid" class="grid grid-cols-7 gap-1">
              ...
              <template x-for="day in daysInFocusedMonth" x-bind:key="day">
                <div x-text="day" x-on:click="dayIsDisabled(day) || selectDate(day)"
                  ...
                  x-bind:dusk="'dasher.forms.{{ $getStatePath() }}' + '.focusedDate.' + day"
          ...
          @if ($hasTime())
            <div @class([
              ...
              <input max="23" min="0" type="number" inputmode="numeric" x-model.debounce="hour"
                @class([
                ...
                ]) dusk="dasher.forms.{{ $getStatePath() }}.hour" />
              ...
              <input max="59" min="0" type="number" inputmode="numeric" x-model.debounce="minute"
                @class([
                    'w-16 p-0 pr-1 text-xl text-center bg-gray-50 text-gray-700 border-0 focus:ring-0 focus:outline-none',
                    'dark:text-gray-200 dark:bg-gray-800' => config('forms.dark_mode'),
                ]) dusk="dasher.forms.{{ $getStatePath() }}.minute" />
              ...
              @if ($hasSeconds())
                ...
                <input max="59" min="0" type="number" inputmode="numeric" x-model.debounce="second"
                  dusk="dasher.forms.{{ $getStatePath() }}.second" @class([
                      'w-16 p-0 pr-1 text-xl text-center bg-gray-50 text-gray-700 border-0 focus:ring-0 focus:outline-none',
                      'dark:text-gray-200 dark:bg-gray-800' => config('forms.dark_mode'),
                  ]) />
              @endif
