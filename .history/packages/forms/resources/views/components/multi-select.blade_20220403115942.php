<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()"
  ...
  <div x-data="multiSelectFormComponent({
    ...
    {{ $attributes->merge($getExtraAttributes())->class([
            'block w-full transition duration-75 divide-y rounded-lg shadow-sm border focus-within:border-primary-600 focus-within:ring-1 focus-within:ring-primary-600 dasher-forms-multi-select-component',
    ...
    @unless($isDisabled())
      <div x-on:click.away="closeListbox()" x-on:blur="closeListbox()"
        x-on:keydown.escape.stop="closeListbox()" class="relative"
        dusk="dasher.forms.{{ $getStatePath() }}.close">
        <div x-ref="button" x-on:click="toggleListboxVisibility()"
          ...
          x-on:keydown.delete="if (! search) clearState()" x-bind:aria-expanded="isOpen"
          aria-haspopup="listbox" tabindex="1" class="relative overflow-hidden rounded-lg">
          <input x-ref="search" x-model="search" x-on:keydown="if (! isOpen) openListbox()"
            x-on:keydown.enter.stop.prevent="selectOption()"
            x-on:keydown.arrow-up.stop.prevent="focusPreviousOption()"
            x-on:keydown.arrow-down.stop.prevent="focusNextOption()"
            placeholder="{{ $getPlaceholder() }}" type="text" autocomplete="off"
          ...
          <span
            class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none rtl:right-auto rtl:left-0 rtl:pr-0 rtl:pl-2">
            ...
            <svg x-show="isLoading" x-cloak class="w-5 h-5 text-gray-400 animate-spin"
        ...
        <div x-ref="listbox" x-show="isOpen" x-transition:leave="ease-in duration-100"
          ...
          <ul x-ref="listboxOptionsList"
            ...
            <template x-for="(key, index) in Object.keys(options)" :key="key">
              <li x-bind:id="'{{ $getName() }}' + 'Option' + focusedOptionIndex"
                x-on:click="selectOption(index)" x-on:mouseenter="focusedOptionIndex = index"
                x-on:mouseleave="focusedOptionIndex = null" role="option"
                ...
                <span x-show="state.indexOf(key) >= 0"
                  x-bind:class="{
            ...
            <div x-show="! Object.keys(options).length" @class([
    ...
    <div x-show="state.length" x-cloak
      ...
      <div class="flex flex-wrap gap-1">
        <template class="hidden" x-for="option in state" x-bind:key="option">
          <button @unless($isDisabled()) x-on:click.stop="deselectOption(option)" @endunless
            type="button" @class([
