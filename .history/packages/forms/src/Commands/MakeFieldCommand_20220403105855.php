...
class MakeFieldCommand extends Command
{
    ...
    public function getArgument($key, $question, $field) : string
    {
        return (string) Str::of($this->argument($key) ?? $this->askRequired($question, $field))
            ->trim('/')
            ->trim('\\')
            ->trim(' ')
            ->replace('/', '\\');
    ...
    public function handle() : int
    {
        $field = (string) Str::of($this->argument('name') ?? $this->askRequired('Name (e.g. `RangeSlider`)', 'name'))
            ...
            ->trim(' ')
            ->replace('/', '\\');
        $dashboard = (string) Str::of($this->argument('dashboard') ?? $this->askRequired('Dashboard (e.g. `dashboard`)', 'dashboard'))
            ->trim('/')
            ->trim('\\')
            ->trim(' ')
            ->replace('/', '\\');
