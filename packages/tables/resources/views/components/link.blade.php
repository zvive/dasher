...
@php
$linkClasses = [
    'inline-flex items-center justify-center hover:underline focus:outline-none focus:underline dasher-tables-link',
...
$iconClasses = \Illuminate\Support\Arr::toCssClasses(['dasher-button-icon w-4 h-4', 'mr-1 -ml-2 rtl:ml-1 rtl:-mr-2' => $iconPosition === 'before', 'ml-1 -mr-2 rtl:mr-1 rtl:-ml-2' => $iconPosition === 'after']);
@endphp
...
@elseif ($tag === 'button')
  <button
    @if ($tooltip) x-data="{}"
            x-tooltip.raw="{{ $tooltip }}" @endif
    type="{{ $type }}" {!! $disabled ? 'disabled' : '' !!} {{ $attributes->class($linkClasses) }}>
    @if ($icon && $iconPosition === 'before')
      <x-dynamic-component :component="$icon" :class="$iconClasses" />
    @endif
