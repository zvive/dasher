...
$getHiddenClasses = function (\Dasher\Tables\Columns\Column $column): ?string {
    if ($breakpoint = $column->getHiddenFrom()) {
        return match ($breakpoint) { 'sm' => 'sm:hidden',  'md' => 'md:hidden',  'lg' => 'lg:hidden',  'xl' => 'xl:hidden',  '2xl' => '2xl:hidden' };
    }

    if ($breakpoint = $column->getVisibleFrom()) {
        return match ($breakpoint) { 'sm' => 'hidden sm:table-cell',  'md' => 'hidden md:table-cell',  'lg' => 'hidden lg:table-cell',  'xl' => 'hidden xl:table-cell',  '2xl' => 'hidden 2xl:table-cell' };
    }
...
<div x-data="{
  ...
  <x-tables::container>
    <div
      ...
      <div
        ...
        {!! !$renderHeaderDiv ? 'x-cloak' : null !!} class="flex items-center justify-between p-2 h-14">
        <div>
        ...
        @if ($isSearchVisible || $isFiltersDropdownVisible)
          <div class="w-full flex items-center justify-end gap-2 md:max-w-md">
            ...
            @if ($isFiltersDropdownVisible)
              <x-tables::filters :form="$getFiltersForm()" :width="$getFiltersFormWidth()"
                class="shrink-0" />
            @endif
          </div>
        @endif
    ...
    <div @class([
      ...
      @if (($records = $getRecords())->count())
        <x-tables::table>
          <x-slot name="header">
            @if ($isSelectionEnabled())
              <x-tables::checkbox-cell>
                <x-slot name="checkbox" x-on:click="toggleSelectRecordsOnPage" x-bind:checked="
            ...
            @foreach ($columns as $column)
              <x-tables::header-cell :extra-attributes="$column->getExtraHeaderAttributes()"
                :is-sort-column="$getSortColumn() === $column->getName()" :name="$column->getName()"
                :alignment="$column->getAlignment()" :sortable="$column->isSortable()"
                :sort-direction="$getSortDirection()" :class="$getHiddenClasses($column)">
                {{ $column->getLabel() }}
          ...
          @if ($isSelectionEnabled())
            <x-tables::selection-indicator :all-records-count="$getAllRecordsCount()"
              :colspan="$columnsCount" x-show="selectedRecords.length">
          ...
          @foreach ($records as $record)
            <x-tables::row :record-url="$getRecordUrl($record)" wire:key="{{ $record->getKey() }}"
              x-bind:class="{
                                'bg-primary-500/10': isRecordSelected('{{ $record->getKey() }}'),
                            }">
              @if ($isSelectionEnabled())
                <x-tables::checkbox-cell>
                  <x-slot name="checkbox" x-model="selectedRecords" :value="$record->getKey()"
                    class="table-row-checkbox"></x-slot>
                </x-tables::checkbox-cell>
              @endif
              ...
              @foreach ($columns as $column)
                ...
                <x-tables::cell :action="$column->getAction()" :name="$column->getName()"
                  :alignment="$column->getAlignment()" :record="$record"
                  :tooltip="$column->getTooltip()" :record-action="$getRecordAction()"
                  :record-url="$getRecordUrl($record)"
                  :should-open-url-in-new-tab="$column->shouldOpenUrlInNewTab()"
                  :url="$column->getUrl()" :class="$getHiddenClasses($column)">
                  {{ $column }}
                </x-tables::cell>
              @endforeach
  ...
  <form wire:submit.prevent="callMountedTableAction">
    ...
    <x-tables::modal
      :id="\Illuminate\Support\Str::of(static::class)->replace('\\', '\\\\') . '-action'"
  ...
  <form wire:submit.prevent="callMountedTableBulkAction">
    ...
    <x-tables::modal
      :id="\Illuminate\Support\Str::of(static::class)->replace('\\', '\\\\') . '-bulk-action'"
      :width="$action?->getModalWidth()" display-classes="block">
      @if ($action)
        @if ($action->isModalCentered())
          <x-slot name="heading">
