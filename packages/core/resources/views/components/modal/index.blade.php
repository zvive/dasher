...
<div x-data="{ isOpen: false }"
  ...
  role="dialog" aria-modal="true" class="{{ $displayClasses }} dasher-modal">
  {{ $trigger }}
  ...
  <div x-show="isOpen" x-transition:enter="ease duration-300" x-transition:enter-start="opacity-0"
    ...
    <button x-on:click="isOpen = false" type="button" aria-hidden="true"
      class="fixed inset-0 w-full h-full bg-black/50 focus:outline-none dasher-modal-close-overlay"></button>
    ...
    <div x-show="isOpen" x-trap="isOpen" x-on:keydown.window.escape="isOpen = false"
      ...
      <div @class([
          'w-full mx-auto p-2 space-y-2 bg-white rounded-xl cursor-default dasher-modal-window',
        ...
        @if ($header)
          <div class="px-4 py-2 dasher-modal-header">
            {{ $header }}
          </div>
        @endif
        ...
        <div class="space-y-2 dasher-modal-content">
          @if ($heading || $subheading)
            <div @class([
              ...
              @if ($heading)
                <x-dasher::modal.heading :id="$id . '.heading'">
                  {{ $heading }}
                </x-dasher::modal.heading>
              @endif

              @if ($subheading)
                <x-dasher::modal.subheading>
                  {{ $subheading }}
                </x-dasher::modal.subheading>
              @endif
            </div>
        ...
        @if ($footer)
          <div class="px-4 py-2 dasher-modal-footer">
            {{ $footer }}
          </div>
        @endif
      </div>
