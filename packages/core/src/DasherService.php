...
class DasherService
{
    ...
    {
        return \auth()->guard(\config('dasher.auth.guard'));
    }

    public function buildNavigation() : array
    {
        /** @var \Dasher\Navigation\NavigationBuilder $builder */
    ...
    public function getNavigation() : array
    {
        ...
        $sortedGroups = $groupedItems
            ...
            ->sortBy(function (?string $group) : int {
                ...
                $sort = \array_search($group, $this->navigationGroups, true);
        ...
        return $sortedGroups
            ->mapWithKeys(fn (?string $group) : array => [$group => $groupedItems->get($group)])
            ->toArray();
    }
    ...
    {
        return $this->themeUrl ?? \route('dasher.asset', [
    ...
    public function getUserAvatarUrl(Model $user) : string
    {
        ...
        if ($user instanceof HasAvatar) {
            $avatar = $user->getDasherAvatarUrl();
        }
        ...
        $provider = \config('dasher.default_avatar_provider');
    ...
    {
        if ( ! \in_array(GlobalSearchProvider::class, \class_implements($provider), true)) {
    ...
    {
        Event::listen(ServingDasher::class, $callback);
    }
}
