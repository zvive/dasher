...
class DasherServiceProvider extends PackageServiceProvider
{
    ...
    protected function discoverResources() : void
    {
        ...
        foreach ($dashboards as $dash) {
            ...
            $resourcePath      = $dash['root_path'] ? $dash['root_path'].'/Resources' : null;
            $resourceNamespace = $dash['namespace'] ? $dash['namespace'].'\\Resources' : null;
            if (!$resourcePath || !$filesystem->exists(\config('dasher.resources.path'))) {
                continue;
            }

            Dasher::registerResources(\collect($filesystem->allFiles($resourcePath))
                ->map(function (SplFileInfo $file) : string {
                    return (string) Str::of($resourceNamespace)
                        ->append('\\', $file->getRelativePathname())
                        ->replace(['/', '.php'], ['\\', '']);
                })
