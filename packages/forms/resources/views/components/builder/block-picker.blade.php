...
<div x-show="isCreateButtonDropdownOpen" x-on:click.away="isCreateButtonDropdownOpen = false"
  x-transition
  {{ $attributes->class([
      'absolute z-20 mt-10 shadow-xl border overflow-hidden rounded-xl w-52 dasher-forms-builder-component-block-picker',
        ...
        <button
          ...
          @if ($icon = $block->getIcon())
            <x-dynamic-component :component="$icon"
              class="mr-2 -ml-1 text-primary-500 w-5 h-5 group-hover:text-white group-focus:text-white" />
          @endif
