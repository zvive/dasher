...
<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()"
  ...
  <div
    {{ $attributes->merge($getExtraAttributes())->class(['flex items-center space-x-1 group dasher-forms-select-component']) }}>
    ...
    <div class="flex-1 min-w-0">
      @unless($isSearchable())
        <select {!! $isAutofocused() ? 'autofocus' : null !!} {!! $isDisabled() ? 'disabled' : null !!} id="{{ $getId() }}"
        ...
        @else
          <div x-data="selectFormComponent({
          ...
          })" x-on:click.away="closeListbox()" x-on:blur="closeListbox()"
            x-on:keydown.escape.stop="closeListbox()" {!! ($id = $getId()) ? "id=\"{$id}\"" : null !!}
            ...
            <div
              ...
              <span x-show="! isOpen" x-text="label ?? '{{ addslashes($getPlaceholder()) }}'"
                @class([
              ...
              @unless($isDisabled())
                <input x-ref="search" x-model="search" x-on:keydown="if (! isOpen) openListbox()"
                  ...
                  x-on:keydown.arrow-down.stop.prevent="focusNextOption()" type="text" autocomplete="off"
                  @class([
