<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()"
  ...
  <div x-data="tagsInputFormComponent({
      state: $wire.{{ $applyStateBindingModifiers('entangle(\'' . $getStatePath() . '\')') }},
  })" id="{{ $getId() }}"
    {{ $attributes->merge($getExtraAttributes())->class(['dasher-forms-tags-input-component']) }}
    {{ $getExtraAlpineAttributeBag() }}>
    <div x-show="state.length || {{ $isDisabled() ? 'false' : 'true' }}"
      @class([
      ...
      @unless($isDisabled())
        <div>
          <input autocomplete="off" {!! $isAutofocused() ? 'autofocus' : null !!} id="{{ $getId() }}"
            list="{{ $getId() }}-suggestions" {!! $getPlaceholder() ? 'placeholder="' . $getPlaceholder() . '"' : null !!} type="text"
            dusk="dasher.forms.{{ $getStatePath() }}" x-on:keydown.enter.stop.prevent="createTag()"
            x-on:keydown.,.stop.prevent="createTag()" x-on:blur="createTag()" x-model="newTag"
          ...
          <datalist id="{{ $getId() }}-suggestions">
            @foreach ($getSuggestions() as $suggestion)
              <template x-if="! state.includes('{{ $suggestion }}')"
      ...
      <div x-show="state.length" x-cloak class="overflow-hidden relative w-full p-2">
        <div class="flex flex-wrap gap-1">
          <template class="hidden" x-for="tag in state" x-bind:key="tag">
            <button @unless($isDisabled()) x-on:click="deleteTag(tag)" @endunless type="button"
              x-bind:dusk="'dasher.forms.{{ $getStatePath() }}' + '.tag.' + tag + '.delete'"
              @class([
