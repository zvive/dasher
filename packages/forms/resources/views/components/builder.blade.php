<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()"
  ...
  <div
    {{ $attributes->merge($getExtraAttributes())->class(['space-y-2 dasher-forms-builder-component']) }}>
    @if (count($containers = $getChildComponentContainers()))
      <ul class="space-y-2" wire:sortable
        wire:end="dispatchFormEvent('builder::moveItems', '{{ $getStatePath() }}', $event.target.sortable.toArray())">
        @foreach ($containers as $uuid => $item)
          <li x-data="{ isCreateButtonDropdownOpen: false, isCreateButtonVisible: false }" x-on:click="isCreateButtonVisible = true"
            x-on:click.away="isCreateButtonVisible = false" wire:key="{{ $item->getStatePath() }}"
                ...
                @unless($isItemMovementDisabled())
                  <button wire:sortable.handle
                    ...
                    type="button" @class([
                        ...
                        'dark:text-gray-200 dark:hover:bg-gray-600 dark:focus:text-primary-600' => config(
                            'forms.dark_mode'
                        ),
                    ])>
                    <span class="sr-only">
  ...
  @if (!$loop->last && !$isItemCreationDisabled() && !$isItemMovementDisabled() && (blank($getMaxItems()) || $getMaxItems() > $getItemsCount()))
    <div x-show="isCreateButtonVisible || isCreateButtonDropdownOpen" x-transition
      class="absolute bottom-0 inset-x-0 -mb-7 z-10 h-12 flex items-center justify-center">
      <div class="relative flex justify-center">
        <button x-on:click="isCreateButtonDropdownOpen = true" type="button"
    ...
    <div x-data="{ isCreateButtonDropdownOpen: false }" class="relative flex justify-center">
      <button x-on:click="isCreateButtonDropdownOpen = true" type="button"
        @class([
            ...
            'dark:bg-gray-800 dark:border-gray-600 dark:hover:border-gray-500 dark:text-gray-200 dark:focus:text-primary-400 dark:focus:border-primary-400 dark:focus:bg-gray-800 dark:focus:ring-offset-0' => config(
                'forms.dark_mode'
            ),
        ])>
      ...
      <x-forms::builder.block-picker :blocks="$getBlocks()" :state-path="$getStatePath()" />
    </div>
  @endif
  </div>
</x-dynamic-component>
