...
<div {{ $attributes->class(['dasher-forms-field-wrapper']) }}>
  ...
  <div class="grid gap-2 sm:grid-cols-3 sm:gap-4 sm:items-start">
    @if (($label && !$labelSrOnly) || $labelPrefix || $labelSuffix || $hint)
      <div
        class="flex items-center justify-between gap-2 sm:gap-1 sm:items-start sm:flex-col sm:pt-2 rtl:space-x-reverse">
        @if ($label && !$labelSrOnly)
          <x-forms::field-wrapper.label :for="$id" :error="$errors->has($statePath)"
            :prefix="$labelPrefix" :required="$required" :suffix="$labelSuffix">
            {{ $label }}
