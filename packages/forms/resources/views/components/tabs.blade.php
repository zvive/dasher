<div x-data="{ tab: '{{ count($getTabsConfig()) ? array_key_first($getTabsConfig()) : null }}', tabs: {{ json_encode($getTabsConfig()) }} }"
  ...
  {{ $attributes->merge($getExtraAttributes())->class([
          'rounded-xl shadow-sm border border-gray-300 bg-white dasher-forms-tabs-component',
  ...
  <div {!! $getLabel() ? 'aria-label="' . $getLabel() . '"' : null !!} role="tablist" @class([
    ...
    @foreach ($getTabsConfig() as $tabId => $tabLabel)
      <button type="button" aria-controls="{{ $tabId }}"
        x-bind:aria-selected="tab === '{{ $tabId }}'"
        x-on:click="tab = '{{ $tabId }}'" role="tab"
