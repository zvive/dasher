<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()"
  ...
  @endif
  <button x-data="{ state: $wire.{{ $applyStateBindingModifiers('entangle(\'' . $getStatePath() . '\')') }} }" role="switch" aria-checked="false"
    ...
    dusk="dasher.forms.{{ $getStatePath() }}" type="button"
    {{ $attributes->merge($getExtraAttributes())->class([
            'relative inline-flex shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500 dasher-forms-toggle-component',
