...
Field classes can be found in the `Dasher\Form\Components` namespace.
...
```php
use Dasher\Forms\Components\RichEditor;
...
```php
use Dasher\Forms\Components\Toggle;
...
In Dasher, you may interact with the `Mask` object in the `mask()` method to configure your mask:
...
Under the hood, masking is powered by [`imaskjs`](https://imask.js.org). The vast majority of its masking features are also available in Dasher. Reading their [guide](https://imask.js.org/guide.html) first, and then approaching the same task using Dasher is probably the easiest option.
...
```php
use Dasher\Forms\Components\BelongsToSelect;
...
```php
use App\Models\App;
use Dasher\Forms\Components\BelongsToManyMultiSelect;
...
```php
use Dasher\Forms\Components\CheckboxList;
...
```php
use App\Models\App;
use Dasher\Forms\Components\BelongsToManyCheckboxList;
...
```php
use Dasher\Forms\Components\Radio;
...
```php
use Dasher\Forms\Components\DatePicker;
use Dasher\Forms\Components\DateTimePicker;
use Dasher\Forms\Components\TimePicker;
...
> Please note, it is the responsibility of the developer to delete these files from the disk if they are removed, as Dasher is unaware if they are depended on elsewhere. One way to do this automatically is observing a [model event](https://laravel.com/docs/eloquent#events).
...
> Dasher also supports [`spatie/laravel-medialibrary`](https://github.com/spatie/laravel-medialibrary). See our [plugin documentation](/docs/spatie-laravel-media-library-plugin) for more information.
...
```php
use Dasher\Forms\Components\MarkdownEditor;
...
```php
use Dasher\Forms\Components\Hidden;
...
```php
use App\Models\App;
use Dasher\Forms\Components\HasManyRepeater;
...
```php
use App\Models\App;
use Dasher\Forms\Components\MorphManyRepeater;
...
```php
use Dasher\Forms\Components\Builder;
use Dasher\Forms\Components\FileUpload;
use Dasher\Forms\Components\MarkdownEditor;
use Dasher\Forms\Components\Select;
use Dasher\Forms\Components\TextInput;
...
```php
use Dasher\Forms\Components\TagsInput;
...
> Dasher also supports [`spatie/laravel-tags`](https://github.com/spatie/laravel-tags). See our [plugin documentation](/docs/spatie-laravel-tags-plugin) for more information.
...
```php
use Dasher\Forms\Components\Textarea;
...
```php
use Dasher\Forms\Components\KeyValue;
...
```php
use Dasher\Forms\Components\ViewField;

ViewField::make('notifications')->view('dasher.forms.components.range-slider')
```
...
{
    protected string $view = 'dasher.forms.components.range-slider';
}
```
